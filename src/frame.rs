...
impl FrameEncoder {
  ...
  fn start_encoding_header(&mut self, opcode: Opcode, payload_len: usize) -> bytecodec::Result<()> {
    let header_size;
...
impl Decode for FrameHeaderDecoder {
  ...
  fn finish_decoding(&mut self) -> bytecodec::Result<Self::Item> {
    ...
    let header = track_assert_some!(self.header.take(), bytecodec::ErrorKind::InconsistentState);
